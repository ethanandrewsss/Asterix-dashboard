/* global React, ReactDOM, Recharts, DASHBOARD_DATA */

(function () {
  // --- Safety checks so you get a visible error instead of a blank page ---
  ...
  if (!window.React || !window.ReactDOM) {
    rootEl.innerHTML =
      "<div style='padding:16px;font-family:Inter,system-ui,sans-serif;color:#c53030;font-weight:600;'>React / ReactDOM failed to load.</div>";
    return;
  }

  if (!window.Recharts) {
    rootEl.innerHTML =
      "<div style='padding:16px;font-family:Inter,system-ui,sans-serif;color:#c53030;font-weight:600;'>Recharts failed to load. Check the script tag in index.html.</div>";
    return;
  }

  if (!window.DASHBOARD_DATA) {
    rootEl.innerHTML =
      "<div style='padding:16px;font-family:Inter,system-ui,sans-serif;color:#c53030;font-weight:600;'>DASHBOARD_DATA is missing. Make sure it’s defined before dashboard-component.js runs.</div>";
    return;
  }

  const { useState, useMemo } = React;

  // Grab recharts components safely
  const {
    LineChart,
    Line,
    XAxis,
  ...
  const Dashboard = () => {
    const fallbackWeek =
      (DASHBOARD_DATA.available_weeks && DASHBOARD_DATA.available_weeks[DASHBOARD_DATA.available_weeks.length - 1]) ||
      "2026-01-05";

    const [selectedWeek, setSelectedWeek] = useState(fallbackWeek);
    ...
    const currentWeekData = (DASHBOARD_DATA.weekly_summary && DASHBOARD_DATA.weekly_summary[selectedWeek]) || {};
    const previousWeekData =
      previousWeek && DASHBOARD_DATA.weekly_summary ? DASHBOARD_DATA.weekly_summary[previousWeek] : null;
    ...
    const employeeData = useMemo(() => {
      const weekData = (DASHBOARD_DATA.weekly_employees && DASHBOARD_DATA.weekly_employees[selectedWeek]) || {};
      return Object.entries(weekData)
        .map(([id, data]) => ({
          id: `Doctor ${id}`,
          emp_id: id,
          total_hours: Number(data.total_time || 0),
          total_tasks: Number(data.tasks_completed || 0),
          shifts: data.timesheet_id || 0,
          tasks_per_hour: Number(data.tasks_per_hour || 0),
          avg_shift_length: Number(data.avg_shift_length || 0),
        }))
        .sort((a, b) => (b[sortBy] || 0) - (a[sortBy] || 0));
    }, [selectedWeek, sortBy]);

    const unitData = useMemo(() => {
      const weekData = (DASHBOARD_DATA.weekly_units && DASHBOARD_DATA.weekly_units[selectedWeek]) || {};
      return Object.entries(weekData)
        .map(([name, data]) => {
          const hours = Number(data.total_time || 0);
          const tasks = Number(data.tasks_completed || 0);
          return {
            name,
            hours,
            tasks,
            efficiency: hours > 0 ? (tasks / hours).toFixed(1) : "0.0",
          };
        })
    ...
    const allWeeksTrend = useMemo(() => {
      return availableWeeks.map((week) => {
        const weekData = (DASHBOARD_DATA.weekly_summary && DASHBOARD_DATA.weekly_summary[week]) || {};
        return {
          week: new Date(week).toLocaleDateString("en-US", { month: "short", day: "numeric" }),
          hours: Number(weekData.total_hours || 0),
          tasks: Number(weekData.total_tasks || 0),
        };
      });
    }, [availableWeeks]);
    ...
    const providerTrendData = useMemo(() => {
      ...
      const trends = (DASHBOARD_DATA.employee_trends && DASHBOARD_DATA.employee_trends[selectedProvider]) || [];
      return trends.map((t) => ({
    ...
    const serviceLineTrendData = useMemo(() => {
      ...
      const trends = (DASHBOARD_DATA.unit_trends && DASHBOARD_DATA.unit_trends[selectedServiceLine]) || [];
      return trends.map((t) => ({
        week: new Date(t.week).toLocaleDateString("en-US", { month: "short", day: "numeric" }),
        hours: Number(t.hours || 0),
        tasks: Number(t.tasks || 0),
    ...
    const MetricCard = ({ title, value, subtitle, trend, icon }) => {
      ...
      return (
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: "0.875rem", color: "#718096", fontWeight: 500, marginBottom: "0.5rem" }}>
              {title}
            </div>
        ...
        <div style={{ maxWidth: "1400px", margin: "0 auto", padding: "2rem 3rem" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "1.5rem", marginBottom: "2.5rem" }}>
            <MetricCard
              icon="⏱"
              title="Total Hours"
              value={currentWeekData.total_hours ? Number(currentWeekData.total_hours).toFixed(1) : "0.0"}
            ...
            <MetricCard
              ...
              value={currentWeekData.total_tasks ? Number(currentWeekData.total_tasks).toLocaleString() : "0"}
              subtitle={`${currentWeekData.avg_tasks_per_hour ? Number(currentWeekData.avg_tasks_per_hour).toFixed(1) : "0.0"} tasks/hour avg`}
              trend={changes.tasks}
            />
            <MetricCard
              icon="$"
              title="Revenue Generated"
              value={`$${((Number(currentWeekData.total_revenue || 0)) / 1000).toFixed(1)}k`}
          ...
          <div style={{ background: "white", padding: "1.75rem", borderRadius: "16px", marginBottom: "2.5rem" }}>
            <h3 style={{ fontSize: "1.125rem", fontWeight: 700, marginBottom: "1.5rem" }}>Overall Performance Trends</h3>
              ...
              {employeeData.map((emp, idx) => (
                ...
                >
                  ...
                  <div style={{ minWidth: "180px" }}>
                    ...
                    <div style={{ fontSize: "0.813rem", color: "#718096" }}>
                      {emp.shifts} shifts · {emp.avg_shift_length}h avg
                    </div>
                  </div>
          ...
          {selectedProvider && providerTrendData.length > 0 && (
            <div style={{ background: "white", padding: "1.75rem", borderRadius: "16px", marginBottom: "2.5rem" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem" }}>
                <h3 style={{ fontSize: "1.125rem", fontWeight: 700 }}>Doctor {selectedProvider} - Week over Week Performance</h3>
                <button
          ...
          {selectedServiceLine && serviceLineTrendData.length > 0 && (
            <div style={{ background: "white", padding: "1.75rem", borderRadius: "16px", marginBottom: "2.5rem" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem" }}>
                <h3 style={{ fontSize: "1.125rem", fontWeight: 700 }}>{selectedServiceLine} - Week over Week Performance</h3>
                <button
  ...
  // --- React 18 mount (THIS is the key change) ---
